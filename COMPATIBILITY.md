...
per-package release tags: `v*` for the server and `python/v*` for the Python
SDK). This table records which SDK releases are verified against which
...
The SDK exposes the server's reported version (from the health endpoint,
`GET /`) and a compatibility check against its declared range. The check is
