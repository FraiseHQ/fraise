---
title: "Grammar"
description: "The complete EBNF grammar of the Fraise Query Language, v1"
navIcon: "📜"
---
...
(* a term: there a keyword starts a clause only when ':' follows   *)
(* it. After it, a keyword starts a clause; quote it ('top') to    *)
(* search for the word itself.                                     *)
...
modifier        = since_field | until_field | depth_field | top_field ;   (* scopes a search, never seeds one *)
...
duration          = integer time_unit ;        (* at most ~292 years, e.g. 106751d: see Bounds *)
time_unit         = 's' | 'm' | 'h' | 'd' | 'w' ;
iso_date          = ?\d{4}-\d{2}-\d{2}? ;         (* date only: ':' would end the token *)
param_ref         = '$' param_name ;
param_name        = bare_word ;                (* matched exactly against a key of the request's parameters: never folded *)
```
