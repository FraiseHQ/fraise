...
The `version` field is the SDK's compatibility handshake.
...
| `Rejecting query, scheduler queue full` warnings | The scheduler queue is saturating and requests are being shed with 429. See [Load and backpressure](/docs/operations/runbook/backpressure). |
