...
those seeds up to `depth:` hops, attenuating the score by `hop-attenuation` per
hop. Text and vector search decide *where* to start; the graph walk decides what
comes with it. This is why a recall needs at least one term: without a seed
there is nowhere to start walking.
